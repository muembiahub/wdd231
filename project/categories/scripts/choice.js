  ...
  window.getLocation = function () {
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(
        ...
        function (error) {
          alert(
            "Position non détectée. Veuillez autoriser l’accès à la localisation.",
          );
        },
      );
    } else {
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function updateCartDisplay() {
    ...
    payBtn.addEventListener("click", () => {
      ...
      alert(
        `✅ Paiement simulé de $${total.toFixed(2)} pour ${cart.length} article(s).\nMerci pour votre commande !`,
      );
  ...
  viewCartBtn.addEventListener("click", () => {
    cartPanel.style.display =
      cartPanel.style.display === "none" ? "block" : "none";
    updateCartDisplay();
  });
  ...
  fetch("scripts/telephones-tablettes.json")
    ...
    .catch((error) => {
      catalog.innerHTML =
        "<p style='color:red'>⚠️ Échec du chargement des produits.</p>";
      console.error(error);
    });
});
